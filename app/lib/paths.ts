...
// Paths, like Activities, are derived from raw events, existing primarily for performance reasons. While they are a lot
// more compact than the underlying events, they still likely contain a lot more data than the Activities themselves.
...
// Note: All these arrays must have the same number of elements!
export interface PathUpdate { // does not extend Realm.Object
  id: string; // should match id of corresponding Activity
  ele: number[]; // elevations in meters
  lats: number[]; // required, may be empty
